...
export const productValidSchema = joi.object({
    title: joi.string().required().trim().regex(/^[a-zA-Z(, \)]*$/).min(3).max(25),
    description: joi.string().required().trim().regex(/^[a-zA-Z(, \)]*$/).min(3).max(100),
    price: joi.number().required(),
    currencyId: joi.string().required().trim().regex(/^[INR]*$/i).messages({"string.pattern.base" : `CurrencyId Should be Indian`}),
...
export const getProducts = joi.object({
    name : joi.string().optional().trim().regex(/^[a-zA-Z(, \)]*$/).min(3).max(25),
